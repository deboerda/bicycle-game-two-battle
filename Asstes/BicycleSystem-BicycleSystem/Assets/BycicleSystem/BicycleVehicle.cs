...
public class BicycleVehicle : MonoBehaviour
{
	...
	public void GetInput()
	{
		...
		// WASD 控制前后左右
		if (Input.GetKey(KeyCode.W)) targetVerticalInput = 1f;
		else if (Input.GetKey(KeyCode.S)) targetVerticalInput = -1f;

		if (Input.GetKey(KeyCode.A)) targetHorizontalInput = -1f;
		else if (Input.GetKey(KeyCode.D)) targetHorizontalInput = 1f;

		// 平滑过渡到目标输入值
		horizontalInput = Mathf.SmoothStep(horizontalInput, targetHorizontalInput, Time.deltaTime * smoothingSpeed);
		vereticallInput = Mathf.SmoothStep(vereticallInput, targetVerticalInput, Time.deltaTime * smoothingSpeed);
		braking = Input.GetKey(KeyCode.LeftShift);
	}
